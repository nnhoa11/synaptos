...
function buildPosSummary(storeRecommendations) {
  return {
    sold_units_last_window: Number(
      storeRecommendations.reduce((sum, recommendation) => sum + (recommendation.lot?.recentVelocity ?? 0), 0).toFixed(2)
    ),
    active_lots: storeRecommendations.length,
    active_markdowns: storeRecommendations.filter(
      (recommendation) => (recommendation.activePrice ?? 0) < (recommendation.lot?.basePrice ?? 0)
    ).length,
  };
}
...
function buildSignalObservation(storeId, sourceType, signal, structuredSignal) {
  ...
  return {
    ...
    provenance: signal.cached ? "cached" : signal.error ? "simulated" : "live",
...
export async function runPipeline({ actorUserId, snapshotKey, storeId, user = null }) {
  ...
  const lots = buildLots(storeRecommendations);
  ...
  const crawledSignals = await crawlSignals({
    storeId: store.id,
    district: store.district,
    category: dominantCategory(storeRecommendations),
  });
  ...
  const aggregationRun = await runAggregationAgent({
    ...
    pos_summary: buildPosSummary(storeRecommendations),
    lots,
